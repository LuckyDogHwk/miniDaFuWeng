...
export default defineConfig<'vite'>(async (merge, _env) => {
  ...
  const baseConfig: UserConfigExport<'vite'> = {
    ...
    defineConstants: {
      ...
      ONLINE_ROOM_CLOUD_ENV: JSON.stringify(
        process.env.TARO_APP_WEAPP_CLOUD_ENV ||
          process.env.WEAPP_CLOUD_ENV_ID ||
          '',
      ),
      ONLINE_ROOM_WS_URL: JSON.stringify(
        process.env.TARO_APP_WEAPP_WS_URL ||
          process.env.ONLINE_ROOM_WS_URL ||
          'ws://127.0.0.1:3000/room',
      ),
