...
### 2026-06-21 迷你大富翁小程序迭代

- 四人/AI 数量问题：用户要求新增两个 AI，但我一开始没有充分确认用户当前预览是否真正显示为三名 AI；以后必须同时检查 store 状态、UI 状态栏、棋子渲染、构建产物和实际预览。
- UI 反复返工问题：多次只根据局部截图或单点要求修改，导致布局遮挡、重叠、大小不一致；以后做棋盘类 UI 先建立统一布局规则，包括节点尺寸、间距、边界、HUD 层级，再实现。
- 编码问题：之前用 PowerShell here-string 写入中文，导致新增中文出现问号；以后改中文文本必须用 UTF-8 明确读写，并在写后搜索连续问号占位、乱码和旧文案。
- 验证不足问题：之前常用类型检查或构建通过就汇报完成，但用户关心的是微信开发者工具里的实际画面；以后 UI 修改必须优先做真实预览或至少明确说明尚未视觉验收。
- 环境一致性问题：扣子预览、下载代码、微信开发者工具、本地工作区之间可能不是同一个构建状态；以后改完要提醒刷新或重新编译，并确认用户正在看的项目路径。
- 微信开发者工具 `dist/game.json` 问题：出现 `simulator game launch error` 时，不要通过新增 `game.tsx/game.config.ts/game.json` 解决；本项目是 Taro 小程序，正确入口是 `dist/app.json` + `pages/index/index`。必须先检查 `project.config.json` 的 `compileType/miniprogramRoot/appid`、`dist/app.json`、工具日志里的 `fetchAttr`，以及 WeappLocalData 是否存在 `isGame/gameApp`。若 AppID 被工具后台识别为小游戏，应更换普通微信小程序 AppID 或临时用 `touristappid` 验证，不能把小程序项目硬改成小游戏入口。
- 联机云开发权限问题：`errCode: -601034` 不是房间业务逻辑错误，而是当前 AppID 没有云开发/数据库权限或未配置云环境。以后做联机前先确认：使用普通小程序 AppID、微信开发者工具中已开通云开发、存在环境 ID、数据库集合和权限规则可写；游客 AppID 只能验证页面，不能验证真实云联机。
# 2026-06-21 联机方案复盘

- 用户使用的是微信小程序测试号，测试号没有云开发能力，不能再用 `wx.cloud.database()` 创建房间。
- 以后遇到测试号、游客 AppID、未开通云开发的 AppID，要优先切换到自建 WebSocket/HTTP 服务。
- 本地测试使用 `ws://127.0.0.1:3000/room` 或局域网 IP；正式发布再换成备案域名下的 `wss://`。
- 不要反复让用户去配置不存在的云开发；先确认 AppID 能力，再决定联机技术路线。
